...
.controller('TopPanelController', ['$rootScope', '$scope', '$mdDialog', '$mdToast', '$$rAF', 'canvas', 'history', 'saver', function($rootScope, $scope, $mdDialog, $mdToast, $$rAF, canvas, history, saver) {
    ...
    $rootScope.cnavasSlides = [];
    $scope.createNewCanvas = function(width, height) {
        canvas.openNew(width, height);
        $scope.closeUploadDialog();
        var canvasId = 'id-canvas-'+ $rootScope.cnavasSlides.length;
        var canvasName = 'name-canvas-'+ $rootScope.cnavasSlides.length;

        $rootScope.cnavasSlides.map(function(val){
            val.active = false;
        });

        $rootScope.cnavasSlides.push({canvasId: canvasId, canvasName: canvasName, active:true});

       //$("#viewport").html('<canvas class="upper-canvas md-whiteframe-z2" width="728" height="546" style="position: absolute; width: 728px; height: 546px; left: 0px; top: 0px; user-select: none; cursor: default;"></canvas>');
